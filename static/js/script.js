jQuery(document).ready(function ($) {
	...
	$(function () {
		...
		$('#tags').tagsinput({
			...
			confirmKeys: [13, 44] // Keycodes for Enter and Comma
		});

		// Add new tag on Enter or Comma keypress
		$('#tags').on('beforeItemAdd', function (event) {
			// Check if the user pressed Enter or Comma
			if (event.keyCode === 13 || event.keyCode === 44) {
				// Prevent the default behavior (adding the tag to the input field)
				event.preventDefault();
				// Get the tag name
				var tagName = event.item;
				// Add the tag to the input field with the "X" option
				$('#tags').tagsinput('add', tagName, { preventPost: true });
			}
		});
		...
		$('#tags').on('itemRemoved', function (event) {
			// Get the tag name
			var tagName = event.item;
			// Remove the tag from the input field
			$('#tags').tagsinput('remove', tagName);
		});
